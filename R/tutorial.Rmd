...
# Advanced Example: Mass Properties Uncertainties

The mass properties of parts in an assembly are typically known only
within some tolerance. The uncertainty in these fundamental values
propagates into the computations of aggregate properties. The literature
provides reference equations for propagating uncertainty; we can add
those to our existing capability in a straightforward extension. As
before, we can simplify the code considerably by expressing the
equations in vector or matrix form.
...
The *get*, *set*, and *update* methods for mass properties uncertainties
are straightforward extensions of their counterparts for mass
properties.

## Combiner

The computed mass properties uncertainties depend on the computed mass
properties, so the uncertainties combiner calls the properties combiner
as a preliminary step and extends the result with uncertainties.

The mass uncertainty computation is straightforward:

``` r
r$σ_mass = sqrt(Reduce(`+`, Map(f = function(v) v$σ_mass^2, vl)))
```

As before, representing the center of mass and its uncertainty as
vectors simplifies the code:
...
This final expression can be coded concisely in R:
...
-   Only gates of AND and OR are permitted.
...
library(readr)
source("fault-tree.R")
fp <- as.data.frame(read_tsv("../faults.tsv", show_col_types = FALSE))
ft <- graph_from_edgelist(as.matrix(read_tsv("../fault-tree.tsv", show_col_types = FALSE)[, c("child", "parent")]))
knitr::kable(fp, digits=2)
```
...
requires only the values to be combined, and is therefore passed only
those values as input. In the case of a fault tree, however, the input
probabilities for an AND gate are multiplied, whereas for an OR gate
they are summed. We can accommodate this situation by using the
`override()` method. `override()` is passed the property table, the id
of the target vertex, and the result of calling `combine()`. We can use
a simple pass-through combiner and perform the actual combining
operation in the override as follows:
...
```{r echo = FALSE}
fr <- rollup(ft, fp, update_fault_props, validate_fault_props_table)
knitr::kable(fr, digits = 4)
```
